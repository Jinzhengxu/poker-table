...
音频走 WebRTC，**浏览器之间点对点直传，不经过服务器**，服务端只转发几 KB 的
握手信令。所以开语音不会增加服务器带宽，也不用开任何额外端口。
...
- 谁在说话，牌桌上的头像会亮绿圈，看一眼就知道。名单里可以单独屏蔽某个人的声音
  （只在你这边生效，比如那位在敲键盘）。
- 默认的 STUN 服务器选的是国内连得上的几家。**对称型 NAT 或部分手机蜂窝网络之间
  打不通**，那种情况只能过 TURN 中转：自己搭个 coturn，填上
  `POKER_TURN_URL` / `POKER_TURN_USERNAME` / `POKER_TURN_CREDENTIAL` 即可。
  打不通的时候名单里会明确显示「连不通」并弹提示，不会静默失败。
...
- **语音是 mesh 拓扑**：上限 8 人，再多就该上 SFU 了（那需要一个媒体服务器，
  和"只依赖 ws"的取舍冲突）。另外没有 TURN 的话，对称型 NAT 之间打不通。
