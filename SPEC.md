...
- 断线的玩家保留座位与筹码；若在牌局中，行动超时按自动动作处理（能过牌就过牌，否则弃牌）。
  **断线期间不发新牌**：下一手不算他，连回来（`hello`）后自动归队。
- 真人**连续 2 手一个动作都没做**（全靠超时代打）就自动坐出，点「回到牌桌」恢复；
  人机超时不算（那是模型慢，不是人不在）。
- 手牌结束后若 `autoNextHand` 为真且**有筹码、未坐出、连着的在座玩家 ≥ 2**，
  且**有人在看**（见 §8.4.6），`autoNextHandMs` 后自动开下一手。
...
{"t":"ping"}
{"t":"presence","visible":true,"active":true} // 页面在不在前台 / 用户刚碰过页面，见 §8.4.6
...
4. **人机说的话里不能有自己的牌。**
   模型手上有底牌也有胜率，放它自由发挥就会边打边解说（线上原话：
   「顶对，该打点价值」「河牌听顺子成花了，看你怎么走」）——那等于在牌桌上亮牌。
   提示词里写了一遍，但提示词只是要求：真正兜底的是 `cleanSay()`，
   命中就把**整句**丢掉，不遮盖也不改写（留半句照样在报牌力）。
   两条路（单轮 / agent）共用这一道关，因为两条路都走 `coerceAction()`。

以上四条各有对应的测试（`test/bot.test.js` 的「安全」小节），改动时不要绕过。
...
| 你的位置 | `positionName()` 从 `buttonSeat` 推导 | 前位/劫位/关煞位/按钮/小盲/大盲；单挑时按钮即小盲 |
...
这条与 §8.4.6 的「没人看就不开牌」是两件事：那个管**暂停**——没人看的时候人机
不会自己接着打、白烧 API；这个管**回收**——真人不打算回来了，桌子还给下一个人。

### 8.4.6 没人看就不开牌（`#pauseReason()`）

一桌人机只要凑够两个就能无限打下去，接了 LLM 就是持续付费。所以每次要自动开
下一手之前都先问一句：**牌桌前面有没有人在看？** 没有就停在这一手结束的状态，
等有人回来再续（`hello`、`presence`、坐回来、补码都会重新触发 `#maybeAutoStart()`）。
进行中的那一手不受影响，正常打完再停，筹码结算不会被截断。

「在看」不能只看连接数。关掉浏览器连接确实会断，但**锁屏、切到别的 App、
电脑上切去别的标签页都不会断**——WebSocket 留着，心跳 ping/pong 由浏览器网络层
自己应答，页面死了也照答。而这三种恰恰是「人走了、桌子还在烧钱」的常态。
所以一个连接要同时满足三条才算观众：

1. 连接还在；
2. 页面在前台——前端在 `visibilitychange` / `pagehide` 时发 `presence{visible}`；
3. `AUDIENCE_IDLE_MS`（10 分钟）内碰过页面——收到任何非脚本消息（动作、聊天、
   入座……）都算；光看不动的观众由前端在点击 / 按键 / 滚动时发 `presence{active:true}`，
   节流 30 秒一次。

快照里 `table.paused` 给出暂停原因：`'nobody'`（没有可见的连接）或 `'idle'`
（有人开着页面但太久没碰），前端把它摆在「N 秒后开始下一手」的位置上。
没汇报过 `presence` 的连接（旧版前端、测试桩）按「看得见、刚操作过」处理。

配合 §7 的两条——掉线不发牌、连续两手超时自动坐出——三层各管一段：
真人还在打就正常打；真人不在了先不发他的牌；连观众都没有了就整桌停。
...
翻牌前范围最强的单一解释变量是位置：同一个人在前位和按钮位的入池率能差一倍
...
## 8.7 题库（`server/eval/spots.js` + `scripts/agent-eval.mjs`）

`npm run eval:spots`。同样**不是运行时的一部分**。

### 8.7.1 它和 8.6 的评测台分工

| | 8.6 评测台 | 8.7 题库 |
| --- | --- | --- |
| 问题 | 赢多少钱（bb/100） | 判断得准不准 |
| 方法 | 自对弈几千副牌 | 20 个固定局面，直接问 |
| 方差 | 巨大，要几万手才显著 | 零 —— 局面是钉死的 |
| 花销 | 不花 API（跑规则策略） | 每题一次决策，agent 模式约 16k token |
| 测得了 agent 吗 | 测不了（`playHand` 是同步的） | 测得了，这就是它存在的理由 |

### 8.7.2 三种模式跑同一套题

`--mode rule` / `--mode single` / `--mode agent` 分别对应纯规则、旧版单轮、
多轮工具循环。三条线跑同一套判据，「这轮改造值多少钱」才有答案；
只报 agent 一个数说明不了任何事情 —— 规则策略在送分题上本来就接近满分。

### 8.7.3 判据的硬性要求

- **能用 `forbid` 就不用 `allow`。** 扑克的最优解经常有争议，明显的错误没有。
  判据写窄了，测出来的是出题人的牌技。
- 怎么打都成立的局面（诈唬点）必须显式标 `soft: true`，不判对错只收指标。
- **成对的题只能有一个变量**：手牌、牌面、底池、要跟的钱、连昵称都相同，
  只有行动序列**或者**画像不同。两个都变，反转就没有归因。
- **成对的题必须先验证它真的会翻转**：紧假设（前 5%）下跟注亏、松假设（前 35%）下赚。
  验不过的不是范围题。第一版的成对题是 AQ 顶对面对河牌大注，出题时想当然
  "对手连开三条街就该弃顶对"，真算一遍才发现 **AQ 对前 5% 也有 70% 胜率** ——
  那道题不存在弃牌点，模型两边都跟是对的，报告却显示"没分开"。
- **「分开了」不等于「打对了」。** 这几个牌面上弃牌只在读成前 5% 时才划算，
  读成前 10% 就该跟，门槛附近很窄。所以成对题一律 `soft`：它回答的是
  "模型对对手的差别有没有反应"，不是"反应得对不对"。
  以上三条 `test/spots.test.js` 都强制。
- 判据看不见的东西要单列成指标。**「不该弃牌」放不住「拿着听牌把 600 推进
  100 的底池」** —— 所以报告里单列开火率和全下率。
- **底池由行动序列推出来，不许手写。** 照 `engine.js#totalPot`：
  前面几条街 + 本轮所有人的 `committedRound`（**包含对手那个还没被跟的注**）。
  手写会飘：第一版 41 道里 26 道的底池和自己的行动序列对不上，而这三样
  （底池、行动序列、座位投入）都逐字进提示词。
- **只统计模型自己答的决策，口径由 `modelAnswered(mode, source)` 一处定义。**
  `agent` 是 `source === 'agent'`，`single` 是 `source === 'llm'`——`BotDriver#decide`
  调用失败时会**静悄悄**改用规则策略，除了 `source` 没有任何别的痕迹。
  混进去算出来的分既不是模型的也不是规则的，而且规则策略从不全下，
  兜底率一高还会把全下率机械地稀释掉。报告里只要有兜底就必须多打一行
  「只算模型自己答的」，`test/bot.test.js` 拿真实驱动的返回值验这条判据。
- **正确率要连样本量一起读。** 33 道判分题，差 1 道就是 3 个百分点；
  两条线的 95% 区间叠了就是分不开，不许拿点估计讲故事。
  能不用判对错就数出来的量（开火率、全下率、成对题分开数、步数、token、延迟）
  比正确率结实得多，结论优先建在那些量上。
- **题库必须对傻子策略有区分度。** 五个不看牌的固定策略（永远跟注/永远弃牌/
  永远全下/永远最小下注/永远加注到最大）的最高分就是这套题的**地板**，
  跑分报告每次都打印它。判据只写「不许弃牌」时地板会涨到 75%，
  那时 95% 和 100% 的差别是噪声。测试卡住地板 ≤ 72%，
  并要求「惩罚进攻」和「惩罚保守」的题各占三成以上。

### 8.7.3b 工具消融（`--exclude`）

`--exclude plan_bet` 把工具从**工具集和系统提示词里一起**摘掉。提示词由工具名生成
（`buildAgentSystem(persona, maxSteps, toolNames)`），所以两边不可能对不上——
少了工具还留着"先调 plan_bet"，模型会去调一个不存在的东西，白烧一步。`act` 摘不掉。

读消融结果的两条硬性要求：

- **只统计真正走通 agent 循环的决策。** 兜底走的是规则策略，它从不全下，
  兜底率一高就把全下率机械地稀释了。两组的兜底率不一样时，直接比总体比率是错的。
- **兜底率本身要先归因再用。** 消融那一轮兜底 32%，但同一道题单独重跑三次全部成功，
  说明是网关抖动不是摘工具造成的——这种数不能写进结论。
- **要下因果结论就配对比，别比总体比率。** 两条线跑的是同一批题同一批遍数，
  按 `(题, 遍)` 配对之后用 McNemar：只在 A 组出现的次数 vs 只在 B 组出现的次数。
  `plan_bet` 那次的开火差别（14 vs 4，双侧 *p* = 0.031）就是这么算出来的；
  只看总体 54% vs 39% 在这个样本量下站不住。
- **注意反向因果。** "调了 `plan_bet` 的决策 70% 开火、没调的 0% 开火"这种相关
  单独看没有方向——也可能是它先想开火才去调工具。方向要靠消融本身给。

### 8.7.4 题库自身要被测

`test/spots.test.js` 离线检查：牌不重复、公共牌张数对得上阶段、
「坚果」题拿真的评牌器验过、聊天内容没漏进提示词、成对题只差一个变量。
理由是这类错误的表现形式是**稳定地判模型错**，而报告上只显示「模型这类题不行」。

### 8.7.5 提示词表面的敏感词扫描

`test/agent.test.js` 里那条离线检查，扫的是所有逐字进请求体的文本：
系统提示词、工具描述、参数描述、每种人数下的每个位置名。
黑名单只收**真在某个网关上撞见过**的词（目前只有一个）。理由见 CHANGELOG：
命中的后果不是报错，是人机静默退回规则策略。
